...
- Gives terse, compressed directives that name only the trigger and the target (e.g. "siapkan", "backend dah siap, siapkan module auth", "siapkan module 'peranti' tu") — often right after being asked an open design question — and expects the agent to work out the scope itself, make a reasonable judgment call on the ambiguity, and flag it for correction rather than blocking on a clarifying question. Confidence: 0.7
